...
class ReportsController extends Controller
{
        ...
        //dd($data);
         ...
         $result = Order::select([DB::raw('DATE_FORMAT("'.Carbon::parse($d_dates['invoice_date'])->format('Y-m-d').'", "%m/%d/%Y") as invoice_date'),
            DB::raw('(CASE WHEN p.practice_code IS NULL THEN '.$invoice_code.' ELSE CONCAT(p.practice_code,"-'.$invoice_code.'") END) AS invoice_number'),
                            ...
                            ->when( (isset(auth::user()->practices->first()->id) && !auth::user()->hasRole('practice_super_group') && !Session::has('practice')), function($query){
                                $query->whereIn('PracticeId', auth::user()->practices->pluck('id'));
                        })->orderBy('Orders.created_at', 'asc')->groupBy('Orders.PracticeId')->get();
                            // dd($result->all());

            if(!Session::has('practice') AND !isset(auth::user()->practices->first()->id) && !auth::user()->hasRole('practice_super_group'))
            {
            ...
            $data['total_transactions'] = $result->count();
            $result->filter(function ($value, $key) {
                    ...
                    $value['for_service_fee'] = $value['base_fee']+$value['profit_share'];
            ...
            $data['total_sum_service_fee'] = '$ '.number_format($result->sum('for_service_fee'), 2);
    ...
    public function searchRxDetails(Request $request)
    {
        ...
        {
            $newList[] = array('simple_date' => date("m-d-Y", strtotime($data)), 'full_date' => $data);
        }
    ...
    public function get_report(Request $request)
    {
       ...
       // var_dump($request->session()->get('practice_names'));
        ...
        {
            $data['report_title'] = 'Rx with Refills Due Soon & Refills Remaining '.$sub_titles;
    ...
    public function refills_due($days, $request)
    {
       ...
       // dd($request->get('practice_ids'));
        ...
        $result = Order::select([DB::raw('DATE_FORMAT(Orders.created_at,"%m/%d/%y") AS service_date'),DB::raw('DATE_FORMAT(Orders.nextRefillDate, "%m-%d-%Y") AS nextRefillDate'),DB::raw('CONCAT(CONVERT(AES_DECRYPT (FROM_BASE64(p.FirstName),"ss20compliance19")USING "utf8"), " ", CONVERT(AES_DECRYPT (FROM_BASE64(p.LastName),"ss20compliance19")USING "utf8")) AS patient_name'),
            ...
            'd.ndc','d.marketer AS marketer','d.gcn_seq','d.major_reporting_cat','d.minor_reporting_class','d.rx_label_name AS rx_label_name','d.dosage_form', 'd.Strength','Orders.DaysSupply AS day_supply','Orders.Quantity', 'Orders.RefillsRemaining AS RefillsRemaining',DB::raw('DATE_FORMAT(Orders.RxExpiryDate,"%m/%d/%y") AS rx_expiry'),'Orders.PrescriberName AS pres_name','Orders.PrescriberPhone AS pres_phone','Orders.InsuranceType AS ins_type',DB::raw('CONCAT("$ ",FORMAT(Orders.RxIngredientPrice,2)) AS ing_cost'),DB::raw('CONCAT("$ ",FORMAT(Orders.RxThirdPartyPay,2)) AS third_pay'),DB::raw('Orders.RxPatientOutOfPocket AS patient_out_pocket'),DB::raw('CONCAT("$ ",FORMAT(Orders.asistantAuth,2)) AS reward_auth'),DB::raw('Orders.asistantAuth/4 AS acti_dis'),  DB::raw('CONCAT(crp.ActivityCount," of 4") AS activity_count'),DB::raw('CONCAT("$ ",FORMAT(Orders.RxFinalCollect,2)) AS selling_price'),DB::raw('Orders.RxProfitability AS rx_profitability'),DB::raw('FORMAT((Orders.RxProfitability/Orders.RxPatientOutOfPocket),2) AS avm'),

        	// DB::raw('CONCAT("$ ",FORMAT(15,2)) AS base_fee'),
            // DB::raw('CONCAT("$ ",FORMAT((0.1*Orders.RxProfitability),2)) AS profit_share'),

            DB::raw('(CASE WHEN Orders.asistantAuth = 0 THEN (SELECT srv.base_fee FROM practices AS prac JOIN services AS srv ON prac.id = srv.practice_id WHERE srv.practice_id = Orders.PracticeId LIMIT 1) ELSE (SELECT srv.fee FROM practices AS prac JOIN services AS srv ON prac.id = srv.practice_id WHERE srv.practice_id = Orders.PracticeId LIMIT 1) END) As base_fee'),
            DB::raw('(CASE WHEN Orders.asistantAuth != 0 THEN ((SELECT srv.profit_share FROM practices AS prac JOIN services AS srv ON prac.id = srv.practice_id WHERE srv.practice_id = Orders.PracticeId LIMIT 1)/100)*(Orders.RxProfitability)  END)  AS profit_share')
            ...
            $result->filter(function($val,$key){
            	$val['service_fee']='$ '.number_format(($val['base_fee']+$val['profit_share']),2);
            	$val['activity_transmit']='$ '.number_format(($val['patient_out_pocket']/4),2);
            	$val['net_phar_profit']='$ '.number_format(($val['rx_profitability']-($val['base_fee']+$val['profit_share'])),2);
            ...
            [ 'title' => 'Service Date', 'style' => "background-color: #808080;color: #FFFFFF;"],
             ...
             ['title'=> 'Patient Out of Pocket','style'=>'background-color: #808080;color: #FFFF00;'],
             ['title'=> 'Reward Authorized','style'=>'background-color: #808080;color: #FFFF00;'],
             ...
             ['title'=> 'Net Pharmacy Profit','style'=>'background-color: #808080;color: #FFFFFF;']
             ];
             ...
             '','','','','','text-align:right;','','','','','','text-align:right;','text-align:right;','text-align:right;color: #FF0000;','text-align:right;color: #FF0000;','text-align:right;background-color: #acb9ca;color: #002060;','background-color: #acb9ca;color: #fff2cc;','text-align: right;','text-align:right;','text-align:right;','text-align:right;','text-align:right;','text-align:right;background-color: #00b04f;color:#FFFFFF;','text-align:right;background-color: #acb9ca;color:#fff2cc;',
    ...
    function accountingExport(Request $request)
    {
        ...
        $result = Order::select([DB::raw('DATE_FORMAT(Orders.created_at, "%m/%d/%Y") as date_service'),DB::raw('(CASE WHEN Orders.RefillCount < 9 THEN CONCAT(Orders.RxNumber,"-0",Orders.RefillCount) ELSE CONCAT(Orders.RxNumber,"-",Orders.RefillCount) END) AS rx_number'),DB::raw('(CASE WHEN Orders.OrderType IS NULL THEN "NEW" ELSE "REFILL" END) AS order_type'),DB::raw('CONCAT(CONVERT(AES_DECRYPT (FROM_BASE64(ppi.FirstName),"ss20compliance19") USING "utf8"), " ", CONVERT(AES_DECRYPT (FROM_BASE64(ppi.LastName),"ss20compliance19")USING "utf8")) AS PatName'),
            DB::raw('DATE_FORMAT(CONVERT(AES_DECRYPT (FROM_BASE64(ppi.BirthDate),"ss20compliance19") USING "utf8"),"%m/%d/%Y") AS dob'),'ppi.Gender AS gender','d.ndc','d.marketer','d.gcn_seq','d.rx_label_name','d.dosage_form','d.strength','Orders.Quantity','Orders.InsuranceType',DB::raw('CONCAT("$",FORMAT(Orders.RxIngredientPrice,2)) AS rxIngcost'),DB::raw('CONCAT("$",FORMAT(Orders.RxThirdPartyPay,2)) AS rx_third_pay'),DB::raw('CONCAT("$",FORMAT(Orders.RxPatientOutOfPocket,2)) AS patient_out_pocket'),'Orders.asistantAuth AS rewardAuth',DB::raw('CONCAT("$",FORMAT(Orders.asistantAuth/4,2)) AS acti_dis'),DB::raw('CONCAT(crp.ActivityCount," of 4") AS activity_count'),'Orders.RxSelling AS rx_selling','Orders.RxProfitability AS rx_profitability',                                
                               DB::raw('(CASE WHEN Orders.asistantAuth = 0 THEN (SELECT srv.base_fee FROM practices AS prac JOIN services AS srv ON prac.id = srv.practice_id WHERE  srv.practice_id = Orders.PracticeId  LIMIT 1) ELSE (SELECT srv.fee FROM practices AS prac JOIN services AS srv ON prac.id = srv.practice_id WHERE  srv.practice_id = Orders.PracticeId  LIMIT 1) END) AS base_fee'),
                               DB::raw('(CASE WHEN Orders.asistantAuth != 0 THEN ((SELECT srv.profit_share FROM practices AS prac JOIN services AS srv ON prac.id = srv.practice_id WHERE  srv.practice_id = Orders.PracticeId LIMIT 1)/100)*(Orders.RxProfitability)  END) AS profit_share'),
                            ...
                            ->when((isset(auth::user()->practices->first()->id) && !auth::user()->hasRole('practice_super_group')), function($query){
                                $query->whereIn('PracticeId', auth::user()->practices->pluck('id'));
            ...
            $result->filter(function ($value, $key) {
                    ...
                    $value['rx_profitability'] = '$ '.number_format($value['rx_profitability'], 2);
                    $value['service_fee'] = $value['base_fee']+$value['profit_share'];
                    $value['rewardAuth'] = '$ '.number_format($value['rewardAuth'], 2);
                    $value['base_fee'] = '$ '.number_format($value['base_fee'], 2);
                    $value['profit_share'] = '$ '.number_format($value['profit_share'], 2);
            ...
            $data['total_sum_service_fee'] = '$ '.number_format($result->sum('service_fee'), 2);
            // dd($data);
            // dd($result->all());
            if(isset(auth::user()->practices->first()->id) && !auth::user()->hasRole('practice_super_group'))
            {

                $data['current_practice']=Practice::find(auth::user()->practices->first()->id);
            ...
            $data['new_query_data'] = $result->all();
            $data['report_title'] = 'Accounting Detail Report';


            return Excel::download(new AccountingExport($data), 'Accounting.xlsx');
    ...
    public function all_cwp_activity_page($dates, $request)
    {
        ...
        $result = DB::table('ActivitiesHistory AS ah')->select(
                ...
                'd.rx_label_name AS rx_label_name', 'd.Strength AS strength', 'd.dosage_form AS dosage_form', DB::raw('FORMAT(o.Quantity ,1) AS quantity'), 'o.RefillsRemaining','d.ndc AS NDC','d.marketer AS Marketer','d.sponsored_product','d.gcn_seq','o.InsuranceType','o.RxSelling AS selling_price','o.RxIngredientPrice AS ing_cost','o.RxThirdPartyPay AS rx_third_pay','o.RxPatientOutOfPocket AS patient_out_pocket','o.asistantAuth AS asistantAuth','o.RxExpiryDate','o.PrescriberName','o.PrescriberPhone','d.major_reporting_cat AS major_cat','d.minor_reporting_class AS minor_rep','o.RxProfitability AS rx_profitability',
                // DB::raw('CONCAT("$ ",FORMAT(15,2)) AS base_fee'),DB::raw('(0.1*o.RxProfitability) AS profit_share'),
                 DB::raw('(CASE WHEN o.asistantAuth = 0 THEN (SELECT srv.base_fee FROM practices AS prac JOIN services AS srv ON prac.id = srv.practice_id WHERE srv.practice_id = o.PracticeId LIMIT 1) ELSE (SELECT srv.fee FROM practices AS prac JOIN services AS srv ON prac.id = srv.practice_id WHERE srv.practice_id = o.PracticeId LIMIT 1) END) As base_fee'),
            DB::raw('(CASE WHEN o.asistantAuth != 0 THEN ((SELECT srv.profit_share FROM practices AS prac JOIN services AS srv ON prac.id = srv.practice_id WHERE srv.practice_id = o.PracticeId LIMIT 1)/100)*(o.RxProfitability)  END)  AS profit_share'),
         ...
         $result->filter(function ($value, $key) {
                    ...
                    $value->net_prac_profit='$ '.number_format(($value->rx_profitability-(($value->base_fee+$value->profit_share)+$value->patient_out_pocket)),2);
           ...
           $data['result'] =  json_decode(json_encode($result), true);
            ...
            ['title'=>'Net Practice Profitability','style'=>'background-color: #808080;color:#FFFFFF']];
             $data['col_style'] = [ 'background-color: #f2f2f2;', 'background-color: #f2f2f2;','background-color: #f2f2f2;','background-color: #f2f2f2;','background-color: #f2f2f2;','background-color: #f2f2f2;','background-color: #f2f2f2;','background-color: #f2f2f2;','background-color: #f2f2f2;','background-color: #f2f2f2;','background-color: #f2f2f2;','','','','','','text-align:right;','text-align:right;','text-align:right;','text-align:right;','text-align:right;color: #FF0000;','text-algin: right;','','','background-color: #d6dce4;color: #0070c0','','text-align:right;','text-align:right;','text-align:right;','text-align:right;background-color: #00b04f;','text-align:right;background-color: #acb9ca;color:#fff2cc;','background-color: #acb9ca;color:#fff2cc','text-align:right;','text-align:right;' ];
         // dd($data);
            return $data;
    }
    ...
    public function all_cwp_users($days, $request)
    {
        ...
        $userList = [];
        $users = User::with(["practice","userRole"])
                ->when($days!='all_activity', function ($query) use ($days) {
                    $end_date = Carbon::now()->format('Y-m-d');
                    $start_date = Carbon::now()->subDay($days)->format('Y-m-d');
        ...
        foreach($users as $user){
            $userList[] = ['name'=>$user['name'], 'email'=>$user['email'], 'role'=>(isset($user['user_role']['name'])?$user['user_role']['name']:""), 'practice'=>(isset($user['practice']['practice_name'])?$user['practice']['practice_name']:""), 'type'=>(isset($user['practice']['practice_type'])?$user['practice']['practice_type']:""),'practice_phone'=> (isset($user['practice']['practice_type'])?$user['practice']['practice_phone_number']:""),'practice_address'=>(isset($user['practice']['practice_type'])?$user['practice']['practice_address']:"")];
        }
    ...
    public function least_medication()
    {
        $data['requests'] = Order::select(['Orders.created_at', 'os.Name as order_status', 'Orders.PatientId AS patient_id', 'Orders.Id AS order_id',
                                    DB::raw('Orders.Rxnumber as RxNumber'), 'Orders.RefillsRemaining', 'Orders.DaysSupply', 
                                    ...
                                    DB::raw('(CASE WHEN d.generic_or_brand="B" THEN d.marketer ELSE d.generic_name END) AS marketer'),
                                    'd.brand_reference AS brand_reference', 'Orders.Quantity AS quantity',
                                    'Orders.asistantAuth AS asisAuth', 'p.practice_name', 
                                    DB::raw('((SELECT SUM(DATEDIFF(nOd.status_updated_date, nOd.nextRefillDateOfPreviousOrder))
                            ...
                            ->join('practices AS p', 'p.id', 'Orders.PracticeId')                                                   
                       ->where(DB::raw('((SELECT SUM(DATEDIFF(nOd.status_updated_date, nOd.nextRefillDateOfPreviousOrder))
                                    FROM Orders AS nOd WHERE nOd.RxNumber=Orders.RxNumber) + 
    ...
    public function greater_sixty_days()
    {
        $data['requests'] = Order::select(['Orders.created_at', 'os.Name as order_status', 'Orders.PatientId AS patient_id', 'Orders.Id AS order_id',
                                    ...
                                    DB::raw('((SELECT SUM(DATEDIFF(nOd.status_updated_date, nOd.nextRefillDateOfPreviousOrder))
                                            FROM Orders AS nOd WHERE nOd.RxNumber=Orders.RxNumber) + 
                                            (DATEDIFF(CURDATE(), DATE_FORMAT(Orders.nextRefillDate, "%Y-%m-%d"))) ) AS total_delay_in_days'),
                                    DB::raw('DATEDIFF(CURDATE(), DATE_FORMAT(Orders.nextRefillDate, "%Y-%m-%d")) AS last_delay')])
                            ->join(
                                    DB::Raw('(SELECT Id, RxNumber, MIN(RefillsRemaining) AS RefillRemain FROM Orders GROUP BY RxNumber) AS my_tables'), 
                                    function($query){
                                    $query->on('my_tables.RxNumber','Orders.RxNumber')
    ...
    public function get_statistics($rx_number)
    {
        $result = Order::select([DB::raw('FORMAT(RxPatientOutOfPocket, 2) AS RxPatientOutOfPocket'), 
                    ...
                    ->where('RxNumber', $rx_number)->where('OrderType', 'Refill')
                    ->where('OrderStatus', 8)
