...
function edit_practice(id)
   {
    // $("#practice-reg-step").find('a[href="#finish"]').attr("href", '#finish-disabled');
      ...
      console.log("******356*****call to the ajax");
       $.ajax({
           type:'GET',
           url:"{{url('/get-practice/')}}/"+id,
           success:function(data) {

               console.log("*********362**********response from the ajax");
               ...
               toastr.clear();
                ...
                $.each(data.users, function( keyu, valu )
                {
                    ...
                    console.log("376************the object value >");
                    console.log(valu)
                    console.log("378********value of the keyu is > " + keyu);

                    
                           if(keyu>0)
                           {
                               console.log("383********CONDITION********value of the keyu is > " + keyu);
                               add_row();
                           }
        /*console.log(keyu);*/
        /*console.log(valu);*/
                           ...
                           if($('#primary_site > .primary_site_div .data-user:nth-child('+(keyu+1)+') > input[type="hidden"]').length)
                           {
                                
                                $('#primary_site > .primary_site_div .data-user:nth-child('+(keyu+1)+') > input[type="hidden"]').remove();
                           }

                           console.log('417*********** using nth child > ' + (keyu+1));

                           console.log("class of nth child is > " + $('#primary_site > .primary_site_div .data-user:nth-child('+(keyu+1)+')').attr('class'));

                           $('#primary_site > .primary_site_div:nth-child('+(keyu+1)+')').prepend('<input fname="id" name="users['+keyu+'][id]" type="hidden" value="" />');
                    
                           $.each(valu, function(innerKey, innerVal)
                           {
                               
                               console.log('426*******key to be used is > ' + innerKey);
                               console.log('427*******value to be used is > ' + innerVal);
                    
                               $('#primary_site > .primary_site_div .data-user:nth-child('+(keyu)+')').find('select[name="users['+(keyu-1)+']['+innerKey+']"] > option[value="'+innerVal+'"]').prop('selected', true);
                               $('#primary_site > .primary_site_div .data-user:nth-child('+(keyu)+')').find('input[name="users['+(keyu-1)+']['+innerKey+']"]').val(innerVal);
...
function add_row()
{
	console.log("441**************ENTERED INTO THE add_row()************");
    var error;
    ...
    var rec_LENGTH = $('.primary_site_div').length;
    
    console.log("461############rec_LENGTH > " + rec_LENGTH);
    console.log("462############add_allow > " + add_allow);

    if(rec_LENGTH < add_allow)
    //if($('.primary_site_div').length < add_allow)
    {
      ...
      plus_minus_btn = plus_minus_btn[0].outerHTML;
        ...
        //if($('.primary_site_div').length==1){ $('.primary_site_div:nth-child(1) > div:last-child').remove(); }

      
        var template_html_element = $('#primary_site > .primary_site_div .data-user').last();
        var template_html = $(template_html_element)[0].outerHTML;

        $('#primary_site > .primary_site_div').append(template_html);

      
        //$('.primary_site_div:last > input[type="hidden"]').remove();
        $('#primary_site:last > input[type="hidden"]').remove();
        ...
        if(($('#primary_site').length < (add_allow + 1) ) && ($('#primary_site').length >= 3))
        {
            var childr = ($('#primary_site').length - 1);
            if($('#primary_site').length>2)
            {
                $('#primary_site:nth-child('+childr+') > div:last-child').remove();
                $('#primary_site:nth-child('+childr+')').append(minus_btn);
            }
            $('#primary_site:nth-child('+$('#primary_site').length+')').append(plus_minus_btn);
    ...
    console.log("519**************EXIT FROM THE add_row()************");
...
function set_index()
{
	console.log("526**************ENTERED INTO THE set_index()************");
    var count = 2;

    $('#primary_site > .primary_site_div .data-user').each(function(index, value){
    ...
    console.log("540**************EXIT FROM THE set_index()************");
}
