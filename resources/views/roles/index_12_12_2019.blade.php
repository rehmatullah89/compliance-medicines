                            ...
                            <div class="practice_search">
                                <div class="row">
                                    <div class="col-sm-12 col-lg-6 col-md-6 float-left">
                                        <h2 class="spacing_left_zero">Roles</h2>
                                    </div>
                                    <div class="col-sm-12 col-lg-6 col-md-6 top-ad-nw-rl-links cnt-right float-right">
                                        <button onclick="window.location='{{ url("roles/create") }}'" class="pull-right"><i class="fa fa-plus"></i>Add New Role</button>
                                        {{--<button onclick="window.location='{{ url("modules") }}'" class="pull-right">Modules</button>--}}
...
{{--                                                <th scope="col">#</th>--}}
                                                    ...
                                                    <th class="perm-col" scope="col">Permission</th>
                                                ...
                                                @foreach($permissions as $value)
                                                    <tr id="role_{{$value->id}}">
                                                        <!--   <th>#</th> -->
                                                        <td class="name-col Name_img">{{ $value->name }} </td>

                                                        <td class="perm-col Name_img">
                                                        ...
                                                        <td class="action-col Name_img">
                                                            <form id="del-form" action="{{ url('roles/'.$value->id) }}" method="POST" style="">
                                                                {{ csrf_field() }}
                                                                {{ method_field('delete')}}
                                                                <a  href="{{ url('roles/'.$value->id.'/edit') }}" class="fa fa-edit btn edit_btn_"><span>Edit</span></a>
                                                                <button id="{{$value->id}}" class="btn fa fa-trash-o trash_btn_" onclick="practice_del(this.id)" ><span>Delete</span></button>
                                                            </form>
...
@section('js')
    <script>
        ...
        function practice_del(id) {
            ...
            var $toast = toastr.error('Do you want remove this Role?<br /><button onclick="delete_record(' + id + ');" type="button" class="btn clear">Yes</button>');
            $toastlast = $toast;
        }
