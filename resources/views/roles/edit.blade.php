...
@section('content')
    <style>
		...
		#roles_create-edit_table thead th.module_td input[type="checkbox"] + span { font-size: 12px; line-height: 1.4em;  }
		...
		#roles_create-edit_table tbody td.module_td input[type="checkbox"] + span { font-size: 12px; line-height: 1.4em; text-transform: uppercase; }
		#roles_create-edit_table tbody td.module_td input[type="checkbox"] + label { font-size: 12px; line-height: 1.4em; text-transform: uppercase; /*color: #242424;*/ cursor: pointer; margin-bottom: 0px;}
    ...
    <div class="row" style="border-bottom: solid 1px #cecece;">
    	<div class="col-sm-12">
    		<h4 class="elm-left fs-24_ mb-3_">Update Roles</h4>
			...
			<div class="row mt-14_ mb-7_">
    			<div class="col-sm-12">
    				...
    				<div class="trow_" id="patient_data">
                        ...
                        <form class="compliance-form_ trow_" method="post" action="{{ url('roles/'.$data->id) }}" id="m_form_1">
                            {!! csrf_field() !!}
                            {{ method_field('PUT') }}
                            ...
                            <div class="flds_rol_nm-lgn_acs flx-justify-start mb-14_">
          						<div class="wd-70p field fld-perm_name mr-1p">
          							...
          							<input type="text" class="wd-100p" name="name" required placeholder="Enter role name" value="{{$data->name}}" maxlength="30">
          							
          						</div>
								
								<div class="wd-30p field fld-perm_name mr-1p">
									...
									<div class="flx-justify-start wd-100p flex-vr-center" style="height: 30px;">
										<div class="radio-field cur-pointer mr-14_">
											<input type="radio" class="size-20_" id="all_access" name="permission[]" value="46" <?=in_array(46,$selected_list)?'checked':''?> style="margin:0px;">
											<label for="all_access" style="margin:0px; line-height: 20px;">All Access</label>
										</div>
										<div class="radio-field cur-pointer mr-14_">
											<input type="radio" class="size-20_" id="practice_admin" name="permission[]" <?=in_array(47,$selected_list)?'checked':''?> value="47" style="margin:0px;">
                            ...
                            <div class="trow_ table-responsive-1024 table-responsive">
                                <table id="roles_create-edit_table" class="table bordered roles_table " style="width: 100%;">
                                    ...
                                    <tr class="weight_500-force-childs bg-blue">
                                        <th class="txt-wht module_td"><input type="checkbox" id="select-all"> <span>Modules </span></th>
                                        <th class="txt-wht bg-grn-lk">Permissions</th>
                                    </tr>
                                    <tr>
                                    <tbody>
                                        ...
                                        <tr class=@if($ind_key % 2 == 0) "bg-gray-f2" @elseif ($ind_key % 2 != 0) "bg-white" @endif>
                                            <td class="module_td text-sline">
                                            	<input type="checkbox" class="module" id="{{ $module->id }}"> <span>{{ $module->name }}</span>
                                            </td>
                                              ...
                                              @if ($permission->module_id == $module->id)
                                                <div class="d-ib_ p4-12_ txt-wht permission">
                                              	  <input type="checkbox" name="permission[]"
                                                       ...
                                                       class="{{ $module->id }} abc"
                                                       data-id="{{ $module->id }}"
                                                       @foreach($selected_permissions as $value)

                                                         @foreach($value->permissions  as $permiss)

                                                           @if($permission->id == $permiss->id)
                            ...
                            <div class="trow_ mt-7_ cnt-left">
                                <button type="button" style="min-width: 120px;"  class="elm-left btn btn-xs btn-danger" onclick="window.location='{{ url("/roles") }}'">Cancel</button>
