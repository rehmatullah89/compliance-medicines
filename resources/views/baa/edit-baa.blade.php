...
<div class="row" id="hidden-opa" >
	<div class="col-sm-12" style="max-width: 1180px; margin: 0px auto; padding-bottom: 100px;">
		...
		<div id="bus-assoc-agrmnts">
            <p class="left-indent-4p">This Business Associate Agreement is executed by and between Compliance Reward, LP. (the "Company") with a principal address of <span class="field-placeholder" style="width: 120px;">{{$practice->businessAgreement->accepted_terms_user}}</span>, and Representative, an individual ("Business Associate") with a principal address of <span class="field-placeholder" style="width: 270px;">{{$practice->practice_address}}</span> to be effective as of the <span class="field-placeholder" style="width: 30px;">{{date('d', strtotime($practice->businessAgreement->accepted_terms_dt))}}</span> day of <span class="field-placeholder" style="width: 20px;">{{date('m', strtotime($practice->businessAgreement->rp_accepted_terms_dt))}}</span>, 20<span class="field-placeholder" style="width: 30px;">{{date('y', strtotime($practice->businessAgreement->accepted_terms_dt))}}</span> (this "Agreement") and is made a part of that certain Marketing Agency Agreement already entered into by and between these same parties dated as of this same Effective Date (the "Marketing Agency Agreement").</p>
        ...
        <div class="trow_"> <!-- form -->
            <div id="bus-assoc-agr-form" class="wd-100p mb-24_ flx-justify-start flex-vr-end">
            	<div class="wd-47p elm-left" style="">
                        ...
                        <label>By:</label>
                         <div id="signature-pad" class="jay-signature-pad">
                            <div class="jay-signature-pad--body mb-7_ br-rds-4" style="width: 100%; height: 100px; border: solid 1px #aaa;">
                                ...
                                @if(!$practice->businessAgreement->crb_accepted_terms)
                                <canvas id="jay-signature-pad br-rds-6 " width=500 height=250 style="width: 100%; height: 100%; border-radius: 7px;"></canvas>
                                @else 
                                <img width=500 height=250 style="width: 100%; height: 100%; border-radius: 7px;" src="{{ asset('storage/'.$practice->businessAgreement->crb_accepted_terms_signature)}}">                              
                              
                              @endif
                        
                        </div>
                            <div class="signature-pad--footer txt-center">                                    <div class="description cnt-left c-dib_ mb-7_"><strong class="fs-17_ weight_600"> CR Signature</strong></div>
                   ...
                   <form id="agreement" action="{{ route('baa_admin') }}" method="POST">
            {{ csrf_field() }}
   ...
   <script src="https://cdn.jsdelivr.net/npm/signature_pad@2.3.2/dist/signature_pad.min.js"></script>
        ...
        <script>
            ...
            savePNGButton.addEventListener("click", function (event) {
                ...
                toastr.error('Please Enter Signature To Ensure the Agreement.');
                return false;
                } else {
